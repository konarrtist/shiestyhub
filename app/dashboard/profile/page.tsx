...
export default async function ProfilePage() {
  ...
  const missingEmbarkId = !profile.embark_id
  ...
  const [{ data: ratingValues }, { data: transactions }, { data: embarkReports }, { data: recentReviews }] =
    await Promise.all([
      ...
      supabase.from("transactions").select("status").or(`buyer_id.eq.${user.id},seller_id.eq.${user.id}`),
      supabase
        .from("embark_reports")
        .select(`
        id,
  ...
  return (
    <div className="space-y-6 pb-6">
      ...
      {missingEmbarkId && (
        <div className="flex flex-col sm:flex-row gap-3 p-4 rounded-lg border border-amber-500/30 bg-amber-500/10">
          <div className="flex items-center gap-2 text-amber-300">
            <AlertTriangle className="h-5 w-5" />
            <span className="font-semibold">Embark ID missing</span>
          </div>
          <div className="flex-1 text-sm text-amber-100">
            Link your Embark username before trading. The platform blocks confirmations until your ID is connected.
            <Link
            ...
            <CardContent>
              {embarkReports && embarkReports.length > 0 ? (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          <p className="text-xs text-slate-500 break-all">
                            Embark ID: {report.reported_embark_id || report.embark_id}
                          </p>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-500 text-xs mt-1">You have no reports against you.</p>
                </div>
              )}
            </CardContent>
          </Card>
